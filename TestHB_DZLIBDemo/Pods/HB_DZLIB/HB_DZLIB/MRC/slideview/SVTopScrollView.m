...
- (void)initWithNameButtons
{
    ...
    for (int i = 0; i < [self.nameArray count]; i++) {
        ...
        int buttonWidth = size.width; //= [title sizeWithFont:button.titleLabel.font\
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [shadowImageView setFrame:CGRectMake(sender.frame.origin.x, 0, [[_buttonWithArray objectAtIndex:self.BUTTONID] floatValue], 44)];
...
{
    float originX = [[_buttonOriginXArray objectAtIndex:BUTTONIDFUN] floatValue];
    float width = [[_buttonWithArray objectAtIndex:BUTTONIDFUN] floatValue];
...
//        [self setContentOffset:CGPointMake(originX - 30, 0)  animated:YES];
        [self setContentOffset:CGPointMake(self.contentOffset.x + 30, 0)  animated:YES];
    }
