...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.font =[UIFont systemFontOfSize:19.0];
        if (self.needboader) {
...
{
    _attributedText =(NSMutableAttributedString *)[attributedText copy];
...
- (void)replaceRange:(UITextRange *)range withText:(NSString *)text
{
    ...
    if (r.range.location + r.range.length <= selectedRange.location) {
        ...
        selectedRange.location = r.range.location;
...
//    self.text = editingText.copy;
    ...
    NSMutableAttributedString *editingAttributedText = [self.attributedText mutableCopy];//一定要用copy
 
//    NSAttributedString *originalAttributedString = [editingAttributedText attributedSubstringFromRange:r.range];
//    attachment.originalAttributedString = originalAttributedString;
    [editingAttributedText replaceCharactersInRange:r.range withString:text];
//    attachment.replacedString = replacementString;
    
//    CTRunDelegateCallbacks callbacks ;
//    CTRunDelegateRef runDelegate = CTRunDelegateCreate(&callbacks, (__bridge void *)originalAttributedString);
//    [editingAttributedText addAttributes:@{(id)kCTRunDelegateAttributeName: (__bridge id)runDelegate} range:r.range];
//    CFRelease(runDelegate);
    self.attributedText = editingAttributedText;
    [self selectionChanged];
}
...
- (void)appendAttributedText:(NSAttributedString *)attributedText
{
    ...
    selectedNSRange.location = editingAttributedText.length;
    selectedNSRange.length = 0;
    [editingAttributedText replaceCharactersInRange:selectedNSRange withAttributedString:attributedText];
    
    self.attributedText = editingAttributedText.copy;
    self.textLayout.markedTextRange = selectedNSRange;//markedTextNSRange;
    self.textLayout.textSelection.selectedRange = selectedNSRange;
...
- (void)insertObject:(id)object size:(CGSize)size;
{
    ...
    if (self.addobj_name.length>0&&self.addobj_type.length>0) {
        attachment.name=self.addobj_name;
        attachment.type=self.addobj_type;
    }
    [self.attachments addObject:attachment];
    [self insertText:replacementCharacter];
    self.addobj_name=nil;
    self.addobj_type=nil;
}
